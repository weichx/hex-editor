...
export class MutableSourceCode {
    ...
    getAst() : ts.SourceFile {
        return this._ast;
    }

    execute(actionList : Array<Action>) : void {

        const fastActions = actionList.filter(action => action instanceof FastAction);
    ...
    translateMap(from : RawSourceMap) : RawSourceMap {
        ...
        const mapConsumer = new SourceMapConsumer(this.getSourceMap());
    ...
    translateDiagnostic(diag : ts.Diagnostic) : ts.Diagnostic {
        const sourceMap : RawSourceMap = this.getSourceMap();
